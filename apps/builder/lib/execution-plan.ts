...
export function FlowToExecutionPlan(
  ...
  const planned = new Set<string>(); //contains all the nodes that are already executed.
  ...
  for (
    ...
    for (const currentNode of nodes) {
      ...
      if (invalidInputs.length > 0) {
        const incomers = getIncomers(currentNode, nodes, edges); //the nodes that are inputs to the current node.
...
function getInvalidInputs(node: AppNode, edges: Edge[], planned: Set<string>) {
  ...
  for (const input of inputs) {
    ...
    if (requiredInputProvidedByVisitedOutput) {
      ...
      //then we need to be that the output is already planned.
      if (!inputLinkedToOutput) continue;
      if (
        inputLinkedToOutput.source &&
        planned.has(inputLinkedToOutput.source)
      ) {
